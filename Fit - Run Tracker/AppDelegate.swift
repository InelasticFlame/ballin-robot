...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func setStateForUITesting() {
        ...
        if ProcessInfo.processInfo.arguments.contains("addRuns") {
            let run1 = Run(runID: 1, distance: 15.4.miles, dateTime: Date(), pace: 9.minutesPerMile, duration: 8316, shoe: nil, runScore: 0.0, runLocations: nil, splits: nil)
            run1.calculateRunScore()
            Database().saveRun(run1)
            let run2 = Run(runID: 2, distance: 5.4.miles, dateTime: Date(), pace: 9.minutesPerMile, duration: 2700, shoe: nil, runScore: 0.0, runLocations: nil, splits: nil)
